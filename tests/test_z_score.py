...
def import_z_score():
    """Attempts to import z_score functions from different possible paths"""
...
class TestZScoreDetection:
    ...
    def test_basic_anomaly_detection(self):
        ...
        data = [100, 102, 98, 101, 99, 500, 101, 98]  # 500 pour un Z-score plus élevé
        anomalies = zscore_detection(data, threshold=2.5)  # Seuil plus bas pour détecter plus d'anomalies
    ...
    def test_numpy_array_input(self):
        ...
        data = np.array([100, 102, 98, 101, 99, 500, 101, 98])
        anomalies = zscore_detection(data, threshold=2.5)
        assert anomalies[5] == True
    
    def test_different_thresholds(self):
        """Test: lower threshold detects more anomalies"""
        data = [100, 102, 98, 101, 99, 300, 101, 98, 500, 100]
        strict = zscore_detection(data, threshold=4.0)
        moderate = zscore_detection(data, threshold=3.0)
        sensitive = zscore_detection(data, threshold=2.5)
        
        assert sum(sensitive) >= sum(moderate) >= sum(strict)
